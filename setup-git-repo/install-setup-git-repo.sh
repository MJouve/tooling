#!/bin/bash
# Script pour installer setup-git-repo de manière globale (accessible depuis partout)
...
LINK_PATH="$LOCAL_BIN/setup-git-repo"
...
ln -s "$SCRIPT_DIR/setup-git-repo.sh" "$LINK_PATH"
...
if [ $? -eq 0 ]; then
    ...
    echo "💡 Vous pouvez maintenant utiliser 'setup-git-repo' depuis n'importe où !"
    echo "   Exemple: cd /mon/projet && setup-git-repo mon-projet"
else
